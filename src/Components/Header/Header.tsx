...
export default function Header() {
    ...
    const changeValue = (e:React.ChangeEvent<HTMLElement> & {target: HTMLInputElement}) => {
    ...
    useEffect(() => {
        if(searchValue) {    
            axios.get(`https://geocoding-api.open-meteo.com/v1/search?name=${searchValue}&count=1&language=ua&format=json`)
            .then(response => {
                if(response.data.results[0].country==="Ukraine") {
                    setSearchResults({name: response.data.results[0].name, id: response.data.results[0].id});
            ...
            <header>
                ...
                <div className="search">
                    <div className="main-search" onClick={() => setSearchOpen(!searchIsOpen)}>
                        ...
                        <span className="select-city">{cityName==="" ? "Вибрати місто" : cityName}</span>
                    ...
                    <div className={`dropdown-search ${searchIsOpen ? "active" : ""}`} ref={menuRef}>
                        <div className="select-city-drop">
                            ...
                            <input type="text" placeholder="Місто, область" onChange={(e) => changeValue(e)} value={searchValue} />
                        </div>
                        <div className="search-results" ref={resultsRef}>
                            <ul className="res-cities-list">
                                <li className="res-cities-list-el"><button onClick={() => {
